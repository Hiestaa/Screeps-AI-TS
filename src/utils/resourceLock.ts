...
export function lock(type: 'constructionSites' | 'structures' | 'resources', room: string, id: string, amount: number, actor: string) {
    logger.info(`${room}: [${type}] locking ${amount} (on id: ${id}, by ${actor})`);
...
export function release(type: 'constructionSites' | 'structures' | 'resources', room: string, id: string, amount: number, actor: string) {
    logger.info(`${room}: [${type}] releasing ${amount} (id: ${id}, by ${actor})`);
    const key = getKey(type, room, id);
    const remain = _.get(Memory, key, 0) - amount;
    _.set(Memory, key, remain);
    if (remain <= 0 && _.get(Memory, key, 'none') !== 'none') {
        logger.info(`${room}: [${type}] no more lock on id: ${id}`);
